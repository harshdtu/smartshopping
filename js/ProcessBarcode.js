$(document).ready(function() {
	...
	$("#default").hide();
	$('#prod-table-list').append( "<li><span class='name'>Cocacola</span><span class='price'> 249</span><span class='remove'><button type='button' class='btn btn-danger'>Remove</button></span></li>" );
	$("#costCalculate").show();
	calculateTotal();
	...
	function addProducts(barcodeValue){
		...
		$(function() {
				...
				buttons: [
					{
						...
						click: function() {
							$( this ).dialog( "close" );
							$('#default').hide();
							$('#prod-table-list').append( "<li><span class='name'>"+ name+ "</span><span class='price'> "+ price +"</span><span class='remove'><button type='button' class='btn btn-danger'>Remove</button></span></li>" );
							$("#costCalculate").show();
							calculateTotal();
						}
						//showText: false
			...
			$('#prod-desc').html("<div><img src='"+imgurl+"'/></div><div>Product: "+name+"</div><div>Desc:"+desc+"</div><div>Price: "+price+"</div>");
		});
	}
